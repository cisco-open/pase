...
class URGENT2Dataset(data.Dataset):
    def __init__(self,
        ...
        self.noise_dic = {}
        for csv in noise_csvs:
            df = pd.read_csv(csv).dropna()
            for line in df.itertuples():
                uid, fs, audio_path = line.uid, line.sample_rate, line.filepath
                assert uid not in self.noise_dic, (uid, fs, audio_path)
                if fs > default_fs:
        ...
        for csv in rir_csvs:
            ...
            for line in df.itertuples():
                uid, fs, audio_path = line.uid, line.sample_rate, line.filepath
                assert uid not in self.rir_dic, (uid, fs, audio_path)
                if fs > default_fs:
                    self.rir_dic[uid] = audio_path

        print(f"Number of {mode} speech:", len(self.speech_dic))
        print(f"Number of {mode} noise:", len(self.noise_dic))
        print(f"Number of {mode} rir:", len(self.rir_dic))
        
        speech_uids = list(self.speech_dic.keys())
        noise_uids = list(self.noise_dic.keys())
        rir_uids = list(self.rir_dic.keys()) + ["none"] * len(self.rir_dic)  # 0.5 prob
        ...
        self.meta = [
            {"id": f"fileid_{i}", "speech_uid": s_uid, "noise_uid": next(noise_cycle), "rir_uid": next(rir_cycle)}
            for i, s_uid in enumerate(speech_uids)
        ]
        # print(f"Number of {mode} dataset:", len(self.meta))
...
if __name__ == "__main__":
    ...
    config = OmegaConf.load('configs/cfg_train_wavlm.yaml')
